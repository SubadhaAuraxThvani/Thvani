...
const CollectionProduct: React.FC<CollectionProductProps> = ({ img,badge, text }) => {
    return (
        <div className="sm:w-[200px] lg:w-full md:w-full bg-white border border-gray-200 rounded-lg shadow">
            <Link href="/product">
            <div className="w-full rounded-t-xl relative overflow-hidden h-[40vh] sm:h-[35vh] lg:h-[45vh]">
             <div className="absolute w-full justify-between px-3 top-2 left-0 items-center flex gap-1 z-10"><div className="flex gap-2">{badge?.map((value:string,ind:number)=>(<Badge key={ind} variant={ind==0?'default':'outline'} className=" lg:text-[.7vw] py-1 h-[1.5vw] rounded-full  pointer-events-none ">{value}</Badge>))}</div> <Button variant="outline" className="bg-transparent rounded-full p-2  aspect-square"><Heart className="w-5 lg:w-[1vw] invert"/></Button></div>
                <Image className="object-cover w-full transform transition-transform duration-300 hover:scale-105" objectFit="cover" src={img} alt="" width={600} height={700} />
            </div>
            </Link>
